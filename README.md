...
The authentication is handled via the API key in the header. The key is generated by Areal.ai and is unique to each user.

In order to authenticate, construct the following phrase and add it to the headers with the "Authorization" key.

```
'Authorization': 'ApiKey Customer_account_id:Customer_key',  # TODO: Update here
```

Customer Account ID is the Username and the Customer Key is the API Key provided by Areal. Both parts are separated by a colon. 

Please contact team@areal.ai to get an API key.
...
```
{
  ...
  "loan_info":{
      ...
      "transaction": {
              "loan_purpose": "purchase",  # Acceptable fields: "purchase", "cash-purchase", "refinance", "cash-out-refi", "no-cash-out-refi", "construction", "construction-perm", "other"
