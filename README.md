...
## Feedback API for Extracted Data

With the Feedback API, you can add, update and delete the extracted data.

### API Endpoint

API endpoint: `https://areal.ai/api/v1/feedback_api/`

Request Method: `POST`

### Authentication

To authenticate, you need to provide the API key in the header.
API Key consists of two parts. The first is your Areal.ai username and the second is the key. Both parts are separated by a colon. The key is generated by Areal.ai and is unique to each user.
...
### Preparation of request payload

The request payload should be a list of dictionaries. Each dictionary can have four keys: `document_id`, `add`, `update` and `delete`.

- `document_id` **(required)**: The document id of the document you want to update. You can find the document id in the URL of the document page.

- `add`: A dictionary that contains the data you want to add. The keys of this dictionary are the group ids and the values are the data you want to add. The data should be in the form of a dictionary. The keys of this dictionary are the input ids and the values are the data you want to add.

- `update`: A dictionary that contains the data you want to update. The keys of this dictionary are the input ids and the values are the data you want to update.

- `delete`: A list of input ids you want to delete.

### Add a new group

Let's say you want to add a new element to the single element group in the image below.

![Single element group](./assets/group.png)

The output below is the current output of this group.
  ...
  "component_results": [
    {
      ...
      "extracted_data": [
        {
          "approved_value": "Michael Jones",
          "confidence_rate": "0.9875",
          "grouped_data": [
            {
              ...
              "extracted_data": [
                {
                  ...
                  "confidence_rate": "0.9875",
                  "grouped_data": [],
                  "height_percent": 0.0101,
                  "page": 0,
                  "processed_value": "Michael Jones",
                  "uuid": "1e03ac98-1815-42ad-844c-03700cc459ad",
                  "width_percent": "0.0866",
                  "x_percent": "0.4510",
                  "y_percent": "0.1237"
                }
              ],
              "name": "01A - Borrower Name",
              "uuid": "f0579f53-c7b7-4665-83ed-97c94a77a67f"
            }
          ],
          "height_percent": 0.0101,
          "page": 0,
          "processed_value": "Michael Jones",
          "uuid": "0d21a335-4261-4a29-b90c-69319ba01d86",
          "width_percent": "0.0817",
          "x_percent": "0.4510",
          "y_percent": "0.1237"
        }
      ],
      "name": "1 - Borrower Name(s)",
      "uuid": "dd2d1833-84ea-4c08-9411-a9c2c31a3263"
    }
  ]
}
```

Now you can use the following raw payload to add a new group.

```json
[
  {
    // Replace this with the document id without curly braces
    "document_id": "{DOCUMENT_ID}",
    "add": {
      "dd2d1833-84ea-4c08-9411-a9c2c31a3263": [
        {
          "approved_value": "",
          "grouped_data": {
            "f0579f53-c7b7-4665-83ed-97c94a77a67f": {
              "approved_value": "New Input Data"
            }
          }
        }
      ]
...
After making the request with the payload above, you will get an image like below in the interface.

![group response](./assets/group-response.png)

### Update a field

You can use the following payload when you want to update a data.
...
[
  {
    ...
    "update": {
      "{EXTRACTED_DATA_UUID}": {
        "approved_value": "Updated Data"
      },
      "{EXTRACTED_DATA_UUID_2}": {
        "approved_value": "Updated Data 2"
      }
    }
  }
]
...
You can use the following payload when you want to delete a data.

```json
[
  {
    "document_id": "{DOCUMENT_ID}",
    "delete": ["{EXTRACTED_DATA_UUID}", "{EXTRACTED_DATA_UUID}"]
  }
]
```

Footnote: You can also add, delete and update operations within the same payload.

### Highlight Areas

You can highlight a field using the Feedback API.

#### Fields

- `approved_value` **(string)**: The value of the field.
- `x_percent` **(number)**: The x coordinate of the top-left corner of the rectangle in percentage.
- `y_percent` **(number)**: The y coordinate of the top-left corner of the rectangle in percentage.
- `width_percent` **(number)**: The width of the rectangle in percentage.
- `height_percent` **(number)**: The height of the rectangle in percentage.
- `from_page_number` **(number)**: The page number of the document where the rectangle starts.

### Example

In the following example, we will add a new group item and highlight it.
      ...
      "{COMPONENT_ID}": [
        {
          ...
          "grouped_data": {
            "f0579f53-c7b7-4665-83ed-97c94a77a67f": {
              "approved_value": "New Input Data With Highlight",
              "x_percent": 0.451,
              "y_percent": 0.1237,
              "width_percent": 0.0817,
              "height_percent": 0.0101,
